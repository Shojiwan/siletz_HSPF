...
# Ryan Shojinaga, PhD Student, Griffith University, Australia Rivers Institute
# shojinaga.ryan@griffithuni.edu.au
# These functions create the initial instance of HSPF model. Specify files and
# file paths -- assumes some files based on folder structure

# Lampert, David. (2015). PyHSPF: Data integration software for hydrologic and
#   water quality modeling. 

# FUNCTION TO CREATE THE INITIAL MODEL INSTANCE
def build_hspf_model(hruF, pcpF, petF, fTab, bsnF, rchF, dir1, strD, endD):
    
    # Assume an hourly timestep
    tstep = 60
 
    # REPLACE WITH READ CODES FROM HEADERS
    lc_codes = ['FORLO', 'FORHI', 'DEVLO', 'DEVHI', 'GRSLO',
                'GRSHI', 'CULLO', 'CULHI', 'IMPRV']
    
    # SPECIFY WDM AND ERROR MESSAGE FILES
    wdmFile  = dir1 + '03_models/01_hspf/siletz_in.wdm'
    mssgpath = dir1 + '03_models/01_hspf/hspfmsg.wdm'
    ...
    fTables = read_ftables(fTab)

    # READ IN BASIN, REACH AND OUTLET SHAPEFILES
    basinRecords = shapefile.Reader(bsnF).records()
    reachRecords = shapefile.Reader(rchF).records()
...
def ts_to_wdmFile(wdmFile, pcpData, petData, mssgpth):
    wdm = WDMUtil(verbose = True, messagepath = mssgpath)
    wdm.open(wdmFile, 'w')
    attributes = {'TCODE ': 3, 
                  'TSSTEP': 1, 
